...
function collectProps(componentNode, ast, input) {
  ...
  function getPropType(node) {
    ...
    if (node.type === 'ArrayExpression') return node.elements.map(el => getPropType(el));
...
function renderProps(props, indent, extraProps) {
  const extraPropsFormatted = extraProps
    .map(prop => prop.split('\n'))
    .reduce((acc, val) => acc.concat(val), [])
    .map((extraProp) => {
      let name = extraProp.split(':')[0].trim();
      let required = true;
      const type = extraProp.split(':').filter((part, index) => index > 0).join(':').trim();
  ...
  extraPropsFormatted.forEach((extraProp) => {
    for (let i = props.length - 1; i >= 0; i -= 1) {
      const prop = props[i];
      if (prop.name === extraProp.name) {
        props.splice(i, 1);
      }
    }
  });

  const propsString = [...props, ...extraPropsFormatted]
    .map((prop) => {
...
function renderMethods(methods, indent, extraMethods) {
  let methodsString = methods
    .map((method) => { // eslint-disable-line
      const methodArguments = method.arguments.map((a) => {
        if ('name' in a && a.name === undefined) return '...args : any[]';
        return `${a.name}? : any`;
      }).join(', ');
      return `${method.name}(${methodArguments}) : unknown`;
    })
    .join(`\n${indent}`);
