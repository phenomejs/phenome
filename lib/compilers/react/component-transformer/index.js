...
function modifyReactClass({ name, componentNode, requiredHelpers }) {
  const thisAliases = ('this that self component').split(' ');
  ...
  traversePhenomeComponent(componentNode, {
    componentWillCreate(node) {
      ...
      reactClassConstructor.value.body.body.unshift(emptyArrowFunction);
    },
  ...
  // Replace .refs access with .__reactRefs
  walk(reactClassNode, {
    MemberExpression(node) {
      if (
        (
        ...
        node.property.name = '__reactRefs';
      }
    },
    VariableDeclarator(node) {
      const isDistructuring = node.id.type === 'ObjectPattern';
      ...
      if (thisAliases.indexOf(init) < 0) return;
      props.forEach((prop) => {
        if (prop.key.name === 'refs') {
          prop.key.name = '__reactRefs';
        }
      });
    },
  });
...
function transform({ ast, name = 'MyComponent', componentNode, state, config, jsxHelpers }) {
  ...
  const { reactClassNode, propsNode, staticProps } = modifyReactClass({ name: camelCaseName, componentNode, config, requiredHelpers });
