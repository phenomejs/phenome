...
function modifyVueComponent({ componentNode, requiredHelpers }) {
  let computed;
  let methods;
  ...
  traversePhenomeComponent(componentNode, {
    static(node) {
      const staticNodeIndex = componentNode.properties.indexOf(node);
      node.value.properties.forEach((prop, index) => {
        componentNode.properties.splice(staticNodeIndex + 1 + index, 0, prop);
      });
      componentNode.properties.splice(staticNodeIndex, 1);
    },
    // eslint-disable-next-line
    props(node) {
      // Skip props processing
    },
    state(node) {
      node.key.name = 'data';
    ...
    shouldComponentUpdate(node) {
      componentNode.properties.splice(componentNode.properties.indexOf(node), 1);
    },
    getSnapshotBeforeUpdate(node) {
      componentNode.properties.splice(componentNode.properties.indexOf(node), 1);
    },
    componentDidCatch(node) {
      node.key.name = 'errorCaptured';
    },
  });
...
function findHelpers({ ast, functional, componentNode, config, jsxHelpers }) {
  const thisAliases = ('this that self component').split(' ');
  ...
  walk(ast, {
    MemberExpression(node) {
      ...
      if (node.object.type === 'Identifier' && node.object.name === 'props' && foundHelpers.indexOf('props') < 0 && !functional) {
  ...
  traversePhenomeComponent(componentNode, {
    ...
    props() {
      if (foundHelpers.indexOf('props') < 0 && !functional) foundHelpers.push('props');
    },
  });
...
function transform({ ast, functional, componentNode, state, config, jsxHelpers }) {
  const requiredHelpers = findHelpers({ ast, functional, componentNode, config, jsxHelpers });

  modifyVueComponent({ componentNode, functional, config, requiredHelpers });
  ...
  // Add props
  if (requiredHelpers.props && !functional) {
